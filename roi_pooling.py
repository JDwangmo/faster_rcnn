...
def roi_pooling(input, rois, size=(7, 7), spatial_scale=1.0):
    ...
    :param input: feature maps input
    :param rois: proposals axis in original image
    :param size: RoI pooling size
    :param spatial_scale: scale which image size /feature size
    :return:
    """
    assert (rois.dim() == 2)
