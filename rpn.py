...
class RPN(nn.Module):
    def __init__(self,
        ...
        :param classifier:
        :param anchor_scales:
        :param negative_overlap: bg_thresh_hi：[0.3]被看做反例样本的anchor与groundtruth的最大IoU
        :param positive_overlap: fg_thresh：[0.7]被看做正例样本的anchor与groundtruth的最小IoU
        :param fg_fraction: [0.5]batch_size中fg样本的比例，如果fg样本个数不足，则添加bg样本
        :param batch_size: [256]每幅图像中筛选使用的bg样本和fg样本的总个数
        :param nms_thresh:
        :param min_size: feat_stride：[16]VGG中conv5_3相比于输入图像缩小了16倍，也就是相邻两个点之间的stride=16
        :param pre_nms_topN:
        :param post_nms_topN:
        """
    ...
    def forward(self, im, feats, gt=None):
        ...
        self._feat_stride = round(im.size(3) / float(feats.size(3)))
        # region rpn
        # put in a separate function
        # cls 和 reg
        rpn_map_pred, rpn_bbox_pred = self.rpn_classifier(feats)
        # 获取在原始图片上的所有anchors
        ...
        if gt is not None:
            ...
            # 所有 anchors 获取标注(2分类, 正/负例) 与 gt boxes的偏移量
            rpn_labels, rpn_bbox_targets = self.rpn_targets(all_anchors, im, gt)
            # need to subsample boxes here
            rpn_loss = self.rpn_loss(rpn_map_pred, rpn_bbox_pred, rpn_labels, rpn_bbox_targets)
        # endregion
        # region roi proposal
        # clip, sort, pre nms topk, nms, after nms topk
        # params are different for train and test
        # proposal_layer.py
        roi_boxes, scores = self.get_roi_boxes(all_anchors, rpn_map_pred, rpn_bbox_pred, im)
        # endregion
    ...
    def rpn_targets(self, all_anchors, im, gt):
        ...
        height, width = im.size()[-2:]
        # only keep anchors inside the image 跨越边界的移除
        _allowed_border = 0
        ...
        assert anchors.shape[0] > 0, '输入图片的分辨率太低了!{0}x{1} -> {2}'.format(height, width, total_anchors)
        ...
        # assign bg labels first so that positive labels can clobber them  IOU小于0.3 为 negative
        labels[max_overlaps < self.negative_overlap] = 0

        # fg label: for each gt, anchor with highest overlap  每个gt boxes 的最大 anchor 为正
        ...
        bbox_targets = bbox_transform(anchors, gt_boxes[argmax_overlaps, :])  # 每个anchor对应的gtboxes
    ...
    def get_roi_boxes(self, anchors, rpn_map, rpn_bbox_deltas, im):
        ...
        # 2. clip predicted boxes to image -- 对超出边界的anchors进行裁边操作
    ...
    def rpn_loss(self, rpn_map, rpn_bbox_transform, rpn_labels, rpn_bbox_targets):
        ...
        :param rpn_map: 预测的anchors分数
        :param rpn_bbox_transform: 预测的anchors偏移量
        :param rpn_labels:
